...
{
    public class NombreCientificoPlant
    {
       public string Value { get;}
        public NombreCientificoPlant(string value)
        {
            if (string.IsNullOrWhiteSpace(value))
                throw new ArgumentException("El nombre científico no puede estar vacío.");
            value = value.Trim();
            if (value.Length > 250)
                throw new ArgumentException("El nombre científico no puede tener más de 250 caracteres.");
            if (!EsNombreValido(value))
                throw new ArgumentException("El nombre científico contiene caracteres inválidos.");
        ...
        {
            if (obj is NombreCientificoPlant other)
            {
